...
bot.command("know", limiter.wrap(async (ctx) => {
  ...
  if (text) {
    ...
    const prompt = trimmedResult
      ? `${text} Be specific. Prefer scientific evidence. Be rational. Knowledge cutoff: ${today} Current date: ${today} This is most recent, online result from the Internet as of ${today}: ${trimmedResult}`
      : text;
