...
bot.command("know", limiter.wrap(async (ctx) => {
  ...
  if (text) {
    ...
    const prompt = trimmedResult
      ? `${text} Be specific. Do not repeat the prompt. Try to base results on scientific evidence. This is what I know from the internet, please summarize it for me: ${trimmedResult}`
      : text;
