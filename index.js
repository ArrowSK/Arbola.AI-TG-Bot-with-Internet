...
bot.command("know", limiter.wrap(async (ctx) => {
  ...
  if (text) {
    ...
    const prompt = trimmedResult
      ? `${text} Be specific. Do not repeat the prompt. Base results on scientific evidence. Be rational. Bear sex in mind. This is current info from the internet, you can use it but do not repeat: ${trimmedResult}`
      : text;
